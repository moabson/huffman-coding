...
void FileManager_decode(FILE *fileCompressedRead, HuffmanTree *huffmanTree,
		FILE *originalFileWriter, int sizeTrash) {
	int currentChar;
	int nextChar;
	int end, i, j;
...
int FileManager_getTrash(FILE *file) {
	// pega primeiro byte do arquivo
	unsigned char character = fgetc(file);
	// mask para o tamanho do lixo
	unsigned char sizeTrash = 0;
	int i, j;
	...
	for (i = 7; i > 4; i--) {
		if (BitwiseOperations_isBitISet(character, i)) {
			sizeTrash = BitwiseOperations_setBit(sizeTrash, j);
		}
...
void FileManager_writeTrash(FILE * fileOutRead, FILE * fileOutWriter,
		int sizeTrash) {
...
int FileManager_writeNewText(FILE * originalFileRead, FILE * fileOutWriter,
...
void FileManager_compressFile(char *fileName) {
	...
	if (file == NULL) {
		...
		printf("~ [%c:%d]\n", huffmanTree->root->character,
				huffmanTree->root->frequency);
		...
		sizeTrash = FileManager_writeNewText(file, fileOutWriter, huffmanTree);

		LOG_INFO("compressed text");

		rewind(fileOutWriter);

		FileManager_writeTrash(fileOutWriter, fileOutRead, sizeTrash);
		...
		printf("The compression rate = %lf%\n",
				(double) ((ftell(file) - ftell(fileOutRead)) * 100)
						/ ftell(file));
	}
...
void FileManager_decompressFile(char *fileName) {
	...
	if (fileCompressed == NULL) {
		...
		if (numberOfNodes == 1) {
			...
			HuffmanTree_rebuilding(fileCompressed, huffmanTree->root,
					huffmanTree);
			...
			FileManager_decode(fileCompressed, huffmanTree, decompress,
					sizeTrash);
